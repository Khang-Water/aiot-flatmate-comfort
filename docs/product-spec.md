...
- Chrome or Edge on localhost is primary browser target.
...
- Dashboard (`/dashboard`): current metrics, common device controls, and hourly-hover charts limited to the latest 24 hours.
...
`microphone active -> wake detected (optional) -> speech captured -> interim/final transcript -> current text flow -> speech output`
