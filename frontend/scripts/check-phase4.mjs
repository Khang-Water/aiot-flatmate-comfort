...
assert.match(voiceHook, /NEXT_PUBLIC_TTS_MODE/);
...
assert.match(voiceHook, /findVietnameseVoice/);
assert.match(voiceHook, /if \(!voice\) \{[\s\S]*?return;[\s\S]*?new SpeechSynthesisUtterance/);
assert.match(voiceHook, /Không tìm thấy giọng đọc tiếng Việt/);
assert.match(dockerfile, /NEXT_PUBLIC_SPEECH_MODE=browser/);
assert.match(dockerfile, /NEXT_PUBLIC_TTS_MODE=backend/);
assert.match(dockerfile, /TTS_ENABLED=true/);
assert.match(dockerfile, /TTS_ENGINE=piper/);
